...
### 1.1. Define Data Structures
    - [X] **Domain Model**: Create `src/domain/models/service.model.ts`.
        ...
        - [X] Add this model to `src/domain/models/models.ts` for aggregated exports.
...
### 1.2. Database Migrations (SQLite for local dev - `MOCK_REPOSITORIES=true`)
    - [X] **Create Table Migration**: Two migration files were created (`20250520125041_create_services_tables.js` and `20250520130000_create_services_tables.js`), leading to a conflict.
    ...
    - [ ] **FIX: Migration Conflict for `services` tables**:
        - [ ] The migration `20250520130000_create_services_tables.js` is failing with "table ... already exists" because `20250520125041_create_services_tables.js` likely already created it.
        - [ ] **Step 1**: Rollback local development migrations until both conflicting `services` table migrations are undone. Use `npm run knex migrate:rollback --env development` (may need to run multiple times). Alternatively, if local data is not critical, reset the SQLite DB file (`src/lib/data/sql/sqlite.db`).
        - [ ] **Step 2**: Consolidate the two migrations. Delete the older or incorrect migration file (e.g., `migrations/20250520125041_create_services_tables.js`). Keep the one with the more robust `down` method (`dropTableIfExists`), which is `20250520130000_create_services_tables.js`.
        - [ ] **Step 3**: Review the chosen migration file (e.g., `migrations/20250520130000_create_services_tables.js`). Ensure the `keywords` column definition is `table.text('keywords')`. This is suitable for SQLite to store a JSON string representation of the `string[]` from the domain model. The `specificType('keywords', 'text[]')` is PostgreSQL syntax and may not behave as a native array in SQLite.
        - [ ] **Step 4**: After cleanup and review, run `npm run migrate:latest` to apply the single, corrected migration.
    - [X] Run `npm run migrate:latest` to apply migrations. (Marked as X assuming it was run, but needs re-running after fix)
    - [ ] (Optional) Update `test_db.js` to query the new service tables for local verification.
    - [ ] **Verify Keywords Column JSON Handling for SQLite**:
        - [ ] In `ServiceLocalRepository`, if `keywords` is stored as a JSON string in SQLite:
            - When reading from DB: `JSON.parse` the `keywords` string to convert it to `string[]`.
            - When writing to DB: `JSON.stringify` the `string[]` `keywords` to store it as a string.
        - [ ] Check `ServiceMapper` to ensure it aligns with how the DTO and Domain model handle `keywords` vs. how the repository persists/retrieves it for SQLite.

### 1.3. Database Migrations (Supabase - if used for prod)
    - [X] **Create Table SQL**: Create an SQL migration file in `supabase/migrations/` (e.g., `20250520133000_create_services_tables.sql`).
        ...
        - For Supabase (PostgreSQL), the `keywords` column should be `TEXT[]`.
    - [ ] **Seed Data SQL **: Add `INSERT` statements in a Supabase SQL migration file for sample data.
