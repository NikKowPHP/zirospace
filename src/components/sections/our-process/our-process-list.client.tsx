...
import { ProcessItem as ProcessItemType } from '@/lib/data/our-processes'
    ...
    >
      ...
      {item.image && (
        <>
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-gray-100 via-gray-100/80 to-transparent" />{' '}
      ...
      <h3
        className="text-xl sm:text-2xl text-black relative z-10 font-medium"
        itemProp="name"
      >
        {' '}
        {/* Adjusted size */}
...
export const ProcessItemListClient = ({
  ...
  useEffect(() => {
    if (processItems[currentIndex]) {
      const timer = setTimeout(() => {
        setLiveRegionText(
          `Showing process step ${currentIndex + 1} of ${numItems}: ${processItems[currentIndex].title}`
        )
      }, 150)
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden px-8 sm:px-16 md:px-24 lg:px-32 xl:px-40">
        {' '}
        {/* Responsive padding */}
        <motion.div
        ...
        >
          {processItems.map((item, index) => (
            <motion.div // Use motion.div for individual slide animation (opacity)
              ...
              animate={{
                // Animate opacity based on index
