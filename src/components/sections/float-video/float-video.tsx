...
import { useVisibility } from '@/contexts/VisibilityContext' // Import the context hook
...
export const FloatVideo = () => {
  ...
  useEffect(() => {
    const handleLoad = () => {
      // Only show video if the OurProcess section is NOT visible initially
      if (!isOurProcessVisible) {
        setShowVideo(true);
  ...
  }, [videoId, isOurProcessVisible]); // Add isOurProcessVisible dependency

  // Conditionally render the entire section based on isOurProcessVisible
  if (isOurProcessVisible) {
    return null; // Don't render anything if the OurProcess section is visible
  }
  ...
  return (
    ...
    <AnimatePresence>
      {!isOurProcessVisible && ( // Conditionally render based on visibility
        <motion.section
          className="fixed bottom-[65px] right-[10px] z-50"
