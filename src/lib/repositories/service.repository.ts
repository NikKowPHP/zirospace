...
export class ServiceRepository implements IServiceRepository {
  ...
  getServices = unstable_cache( // Wrapped with unstable_cache
    async (locale: string): Promise<Service[]> => {
      const tableName = this.getTableName(locale);
      logger.log(`Fetching services for locale: ${locale} (WITH unstable_cache)`);
  ...
  async getServiceById(id: string, locale: string): Promise<Service | null> {
    ...
    return unstable_cache(
      async () => {
        const tableName = this.getTableName(locale);
        logger.log(`Fetching service by id "${id}" for locale: ${locale} (WITH unstable_cache)`);
