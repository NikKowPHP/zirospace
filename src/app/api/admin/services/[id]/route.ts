...
export async function GET(
  ...
  try {
    ...
    const service = await serviceService.getServiceById(validatedId, validatedLocale as Locale);
    if (!service) {
...
export async function PUT(
  ...
  try {
    ...
    const updatedService = await serviceService.updateService(id, domainData, locale as Locale);
...
export async function DELETE(
  ...
  try {
    ...
    const deletedService = await serviceService.deleteService(validatedId, validatedLocale as Locale);
    if (!deletedService) {
      return NextResponse.json({ error: 'Service not found' }, { status: 404 });
    }
