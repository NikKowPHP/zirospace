...
export default function YoutubeAdminPage() {
  const [youtubeSection, setYoutubeSection] = useState<YoutubeModel | null>(null);
  ...
  const handleSubmit = async (data: Partial<YoutubeModel>) => {
    try {
      ...
      toast.error(`Failed to update YouTube section: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setLoading(false);
    }
  };
