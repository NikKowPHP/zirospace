...
const UpdateForm: React.FC<UpdateFormProps> = ({ update, onSubmit, onCancel, loading }) => {
  const {
    ...
    handleSubmit,
    formState: { errors },
    setValue, // To set initial values for RHF if needed, though defaultValues is better
  } = useForm<Partial<Omit<Update, 'content_html' | 'excerpt' | 'publish_date' | 'is_published'>>>({
    defaultValues: {
      title: update?.title || '',
      slug: update?.slug || '',
  ...
  const [isPublished, setIsPublished] = useState(Boolean(update?.is_published) || false);
  ...
  useEffect(() => {
    if (quillContent) {
      ...
      if (update?.content_html && quillContent.root.innerHTML !== update.content_html) {
        quillContent.clipboard.dangerouslyPasteHTML(update.content_html);
      } else if (!update?.content_html && quillContent.root.innerHTML !== '<p><br></p>') {
  ...
  useEffect(() => {
    if (quillExcerpt) {
      ...
      } else if (!update?.excerpt && quillExcerpt.root.innerHTML !== '<p><br></p>') {
  ...
  const handleFormSubmit = async (data: Partial<Omit<Update, 'content_html' | 'excerpt' | 'publish_date' | 'is_published'>>) => {
    await onSubmit({
      ...data,
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
      ...
      <div>
        ...
        <div style={{ width: '100%', height: 150, border: '1px solid #ccc', borderRadius: '4px' }}>
      ...
      <div className="pt-10"> {/* Added some padding top to separate from excerpt */}
        <Label htmlFor="content_html">Content HTML</Label>
        <div style={{ width: '100%', height: 400, border: '1px solid #ccc', borderRadius: '4px' }}>
      ...
      <div>
        ...
        <Input
          ...
          className="w-full"
        />
        {errors.image_alt && <p className="text-red-500 text-sm mt-1">{errors.image_alt.message}</p>}
      </div>
      ...
      <div className="flex items-center space-x-2">
        ...
        />
        <Label htmlFor="isPublished" className="cursor-pointer">Is Published</Label>
      </div>
      ...
      <div>
        ...
        <Input
          type="number"
          id="orderIndex"
          {...register('order_index', { valueAsNumber: true, min: { value: 0, message: "Order index cannot be negative"} })}
          className="w-full"
        />
        {errors.order_index && <p className="text-red-500 text-sm mt-1">{errors.order_index.message}</p>}
      </div>

      <div className="flex justify-end pt-8 space-x-3"> {/* Reduced pt from 20, added space-x */}
