...
export const useAdminUpdates = (initialUpdates?: Record<Locale, Update[]>): UseAdminUpdates => {
  const [updates, setUpdates] = useState<Record<Locale, Update[]>>(initialUpdates || ({} as Record<Locale, Update[]>));
  const [loading, setLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const adminApi = useAdminApi();

  const fetchUpdates = useCallback(async (locale: Locale) => {
    setLoading(true);
    setError(null);
    try {
  ...
  const createUpdate = useCallback(async (data: any, locale: Locale) => {
      ...
      }),
      {
        ...
        success: (newUpdate) => {
          setUpdates((prevUpdates) => {
            ...
            return { ...prevUpdates, [locale]: [...currentUpdates, newUpdate] };
          });
  ...
  const updateUpdate = useCallback(async (id: string, data: any, locale: Locale) => {
    return toast.promise(
  ...
  const deleteUpdate = useCallback(async (id: string, locale: Locale) => {
    return toast.promise(
      adminApi.callApi<void>(`/api/admin/updates/${id}`, {
        method: 'DELETE',
      }),
      {
        ...
        success: () => {
          setUpdates((prevUpdates) => {
            const currentUpdates = prevUpdates[locale] || [];
            const updatedUpdates = currentUpdates.filter((update) => update.id !== id);
            return { ...prevUpdates, [locale]: updatedUpdates };
          });
